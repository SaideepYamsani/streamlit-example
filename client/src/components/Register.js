...
function Register() {
  ...
  const [Role, setRole] = useState("Seller"); // Default Role is set to User
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        const res = await axios.post("http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/signup", {
          UserName: username,
          Email: email,
          Password: password,
          Role: Role,
