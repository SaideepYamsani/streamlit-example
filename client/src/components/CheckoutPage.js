      ...
      const fetchProducts = async () => {
        try {
          ...
          const productPromises = productIds.map((productId) =>
            axios.get(`http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/api/products/${productId}`)
          );
  ...
  const handleCheckout = async () => {
    try {
      const response = await axios.post(
        "http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/user/create-checkout-session",
        {
          cart,
        }
      );
        ...
        <tbody>
          {products.map((product) => (
            <tr key={product._id}>
              ...
              >
                <img
                  src={`http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/${product.image}`}
                  alt={product.name}
