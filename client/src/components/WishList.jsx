...
const WishList = ({ addToCart }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/user/listwishlist/${userID}`
        );
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      await axios.delete(`http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/user/deletewishlist/${itemId}`);
                ...
                <tr key={item._id}>
                  ...
                  <td>
                    <img
                      src={
                        item._id
                          ? `http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/${item.image}`
                          : item.image
                      }
                      alt={item.name}
                      style={{ width: "5rem" }}
