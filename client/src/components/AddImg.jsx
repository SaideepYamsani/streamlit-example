...
function AddImg({ productId }) {
  ...
  const handleDeleteProductImg = async (imageId) => {
    try {
      const response = await axios.delete(`http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/admin-seller/productsImg/${imageId}`, {
        headers: {
  ...
  const fetchProductImages = async () => {
    try {
      const res = await axios.get(`http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/admin-seller/getallproductimgs/${productId}`);
      if (res.data.success) {
    ...
    if (validateForm()) {
      try {
        ...
        const res = await axios.post(
          `http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/admin-seller/productsImage/${productId}`,
          formDataToSend,
          { headers }
        );
          ...
          {productImages.map((image, index) => (
            <div className="col-lg-4 col-md-6 mb-4" key={image._id}>
              <div className="bg-image hover-overlay ripple shadow-1-strong rounded position-relative">
                <img src={`http://ec2-3-21-35-199.us-east-2.compute.amazonaws.com:8000/${image.image}`} alt="" className="w-100 h-auto img-cover" />
                ...
                <div className="mask d-flex justify-content-center align-items-center">
                  <div className="text-white position-absolute top-50 start-50 translate-middle">Image Name ooooooo</div>
                </div>
              </div>
            </div>
          ))}
