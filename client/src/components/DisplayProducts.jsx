...
export default function DisplayProducts({
  ...
  return (
    <div>
      <h1>{Role === "LocalOwner" ? "Local Business Owner " : "Seller "}Dashboard</h1>
      <div style={{ display: 'flex', flexWrap: 'wrap' }}>
        {products && products.length > 0 ? (
          products.map((product) => {
          
            if (
              (Role === 'LocalOwner' && product.seller._id === userID && product.category === 'Local business') ||
              (Role === 'Seller' && product.seller._id === userID && (product.category === 'Clothing' || product.category === 'Food'))
            ) {
              return (
                <div
                  key={product.id}
                ...
                >
                  ...
                  <div className="card-body">
                    ...
                    <p style={{fontSize:"12px"}}>Product Price: {product.price}$</p>
              ...
              return null; // Skip rendering for products that don't match role and category criteria
            }
          })
        ) : (
