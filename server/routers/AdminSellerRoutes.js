...
router.post(
  ...
  async (req, res) => {
    try {
      const { name, price, description, quantity, category, sizeProduct, spice } = req.body;
...
router.patch(
  ...
  async (req, res) => {
    try {
      const { name, price, description, quantity, image, category , sizeProduct, spice} = req.body;
      const product = await ProductModel.findById(req.params.id);
      const newProduct = await ProductModel.findByIdAndUpdate(
        req.params.id,
        {
